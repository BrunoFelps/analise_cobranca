...
-- Agrega informações das faturas por CPF
faturas_agregadas AS (
    SELECT 
        rf.CPF_do_RF,
        MIN(f.Data_de_Vencimento) AS Vencimento_Mais_Antigo,
...
SELECT 
    ...
    CASE WHEN status = 'Erro retornado ao tentar negativar' THEN 1 ELSE 0 END AS `Erro retornado ao tentar negativar`,
    CASE WHEN status = 'Desnegativação solicitada' THEN 1 ELSE 0 END AS `Desnegativacao solicitada`,
